...
contract('GOWToken', function(accounts){
    ...
    it('initializes the contract with the correct vlaues', function(){
        return GOWToken.deployed().then(function(instance){
            tokenInstance=instance;
            return tokenInstance.name();
        }).then(function(name){
            assert.equal(name,'GOW Token','has the correct name')
            return tokenInstance.symbol();
        }).then(function(symbol){
            assert.equal(symbol,'GOW','has the correct token')
            return tokenInstance.standard();
        }).then(function(standard){
            assert.equal(standard,'V 1.0','has the correct standard')
        });
    });
    ...
    it('transfer token ownership', function(){
        return GOWToken.deployed().then(function(instance){
            ...
            assert.equal(receipt.logs[0].args._to, accounts[1], 'logs the account the tokens are transfererd to');
    ...
    it('approves tokens for delegated transfer', function(){
        return GOWToken.deployed().then(function(instance){
            ...
            assert.equal(receipt.logs[0].args._owner, accounts[0], 'logs the account of theowner');
            assert.equal(receipt.logs[0].args._spender, accounts[1], 'logs the account of the spender');
    ...
    it('handles delegated token traansfer',function(){
        GOWToken.deployed().then(function(instance){
            tokenInstance=instance;
            ...
            //Try transferring something larger than the approved amt
            ...
            assert.equal(receipt.logs.length, 1, 'triggers one event');
            assert.equal(receipt.logs[0].event, 'Transfer', 'should  be the "Approve" event');
            ...
            assert.equal(balance.toNumber(),90,'deducts token number from the sending account');
            return tokenInstance.balanceOf(toAccount);
        }).then(function(balance){
            assert.equal(balance.toNumber(),10,'adds token number to the receiving account');
